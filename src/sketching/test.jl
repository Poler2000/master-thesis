module Experiments
    ...
    matrix = generate_symmetric_random_matrix(n, density, max_value)

    #matrix = load_matrix_mat("$DATA_FOLDER$dataset.mat")#[1:1500, 1:1500]

    alpha_sim = 0.5
    alpha = 0.002
    k = 2
    sketch_dimensions = 128
    sample_size = count(!iszero, matrix)
    ...
    matrix_str = join([join([@sprintf("%.4f", element) for element in row], "\t") for row in eachrow(dense_matrix[1:8, 1:16]')], "\n")
    ...
    #flat_A = [element for element in flat_A if element > 0]
    ...
    println("total number of edges: $sample_size")
end
#143