...
\section{Metody oparte na modyfikacji macierzy sąsiedztwa}
    ...
    \begin{figure}[htb]
        ...
        \end{tikzpicture}
        \caption{\textcolor{red}{Graf wraz z odpowiadającymi wierzchołkom wartościami haszy, podpisami oraz adresami. W tym wypdaku ustalono $F = 8$, $m = 4$ oraz $M = 32$. Przykład został zaczerpnięty z \cite{Gou_Zou_Zhao_Yang_2019}.}}
        \label{fig:gss_graph}
        \end{figure} 
            ...
            \begin{algorithm}
                ...
                \uIf{$k > 2$}{
                    ...
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        ...
                        $\color{red} S_{j}^{r}(k) \gets \argmin_{i \in \{1,2,\dots, D\}} \frac{-\log h_{j}(i)}{\tilde{V}_{i}^{r}(k)}, j \in \{1,2, \dots, L\}$\;
                    }
                }
                \ElseIf{$k = 2$}{
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        $\color{red} S_{j}^{r}(2) \gets \argmin_{i \in \{1,2,\dots, D\}} \frac{-\log h_{j}(i)}{\tilde{V}_{i}^{r}(2)}, j \in \{1,2, \dots, L\}$\;
                    }
                }
                \Return{$S(k)$}
            \end{algorithm}
