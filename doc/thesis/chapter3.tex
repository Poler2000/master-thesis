...
\section{Metody oparte na modyfikacji macierzy sąsiedztwa}
    ...
    Strukturą opartą na koncepcie podobnym do TCM jest \emph{Graph Stream Sketch} (GSS)\cite{Gou_Zou_Zhao_Yang_2019}. Celem autorów było stworzenie metody oferującej lepszą skalowalność dla wielkich grafów. Podobnie jak w TCM, funkcja haszująca mapuje zbiór wierzchołków na pewien mniejszy zbiór $M$-elementowy. \textcolor{red}{Wymiary skompresowanej macierzy sąsiedztwa wynoszą notomiast $m \times m$, $m < M$}. Główną zmianą \textcolor{red}{względem TCM} jest wprowadzenie dodatkowych cech opisujących wierzchołki. \textcolor{red}{Pierwszą z nich jest podpis $f(v), 0 \leq f(v) < F$ wierzchołka $v$, gdzie $F$ jest stałą wyznaczającą maksymalną wartość liczbową podpisu, przy czym $F$ i $m$ są dobierane tak, aby spełnić $M = m \times F$. Dla każdego wierzchołka, jego podpis jest wyznaczany na podstawie wartości funkcji haszującej $H(v)$ jako:
    \[
        f(v) = H(v) \bmod F. 
    \] 
        ...
        \begin{tikzpicture}
            ...
            \node [right=2.5cm of a] (adj_table) {
              \begin{tabular}{|c|c|c|c|c|c|c|c|}
                ...
                f(v) & 2 & 7 & 5 & 4 & 2 & 2 & 5 \\ \hline
                h(v) & 0 & 1 & 0 & 3 & 1 & 2 & 0 \\ \hline
              \end{tabular}
            };
        \end{tikzpicture}
        \caption{\textcolor{red}{Graf wraz z odpowiadającymi wierzchołkom wartościami haszy, podpisami oraz adresami. W tym wypadku ustalono $F = 8$, $m = 4$ oraz $M = 32$. Przykład został zaczerpnięty z \cite{Gou_Zou_Zhao_Yang_2019}.}}
        \label{fig:gss_graph}
        \end{figure} 
...
\section{Znurzenia grafów}
    ...
    \subsection{Próbkowanie}
        ...
        Znanym przykładem tego typu techniki jest DeepWalk\cite{Perozzi_Al-Rfou_Skiena_2014}, wywodzący się, jak sama nazwa wskazuje, z uczenia głębokiego (ang. \emph{deep learning}). Wykorzystuje on sekwencję krótkich, losowych spacerów po wierzchołkach grafu do zapamiętywania ich tzw. reprezentacji społecznej \textcolor{red}{(ang. \emph{social representation})}, zawierającej informacje o podobieństwie sąsiedztwa wierzchołków i przynależności do \textcolor{red}{większych klastrów}. DeepWalk wykorzystuje algorytm SkipGram\cite{mikolov2013efficient} do efektywnego wyznaczania wektorów cech. Co ciekawe, można na ten schemat działania patrzeć jak na uogólnienie metod przetwarzania języka naturalnego do przetwarzania języka losowych spacerów, traktowanych jako zdania.
    ...
    \subsection{Szkice}
        ...
        \subsubsection*{NodeSketch}
            ...
            \[
                Pr(S_j = i) = \frac{V_i}{\color{red} \sum_{n} V_n}.  
            ...
            \begin{algorithm}
                ...
                \uIf{$k > 2$}{
                    ...
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        $\color{red} \tilde{V}_{i}^{r}(k) \gets \tilde{V}_{i}^{r} + \sum\limits_{n \in 	\Gamma(r)} \frac{\alpha}{m} \sum\limits_{j = 1}^{m} \mathbbm{1}_{[S_{j}^{n}(k - 1) = i]}, i \in \{1,2, \dots, D\}$\;
                        $\color{red} S_{j}^{r}(k) \gets \argmin_{i \in \{1,2,\dots, D\}} \frac{-\log h_{j}(i)}{\tilde{V}_{i}^{r}(k)}, j \in \{1,2, \dots, m\}$\;
                    }
                }
                \ElseIf{$k = 2$}{
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        $\color{red} S_{j}^{r}(2) \gets \argmin_{i \in \{1,2,\dots, D\}} \frac{-\log h_{j}(i)}{\tilde{V}_{i}^{r}(2)}, j \in \{1,2, \dots, m\}$\;
                    }
                }
                \Return{$S(k)$}
            \end{algorithm}
        ...
        \subsubsection*{SGSketch}
            ...
            Niech $SG$ będzie strumieniowanym grafem. Typowa implementacja stopniowego zapominania krawędzi \textcolor{red}{polega na obliczaniu jej wagi bazując na momencie wystąpienia w strumieniu. Starszym krawędziom przypisywane będą mniejsze wartości, zgodnie z prawem rozpadu naturalnego (ang. \emph{exponential decay})}. Możemy więc przyjąć wagę krawędzi, która pojawiła się w chwili $t$ jako $w_t = e^{\lambda(t_n - t)}$, gdzie $t_n$ jest momentem wystąpienia najnowszej krawędzi, a $\lambda$ jest parametrem sterującym szybkością zapominania. Stąd elementy macierzy sąsiedztwa wyznaczane są ze wzoru:
            ...
            \begin{algorithm}
                ...
                \uIf{$k > 2$}{
                    ...
                    \ForEach{$r \in \Omega$}{
                        $\color{red} \tilde{V}^{r}(k) \gets \tilde{V}_{i}^{r} + \sum\limits_{n \in 	\Gamma(r)} \frac{\alpha}{m} \sum\limits_{j = 1}^{m} \mathbbm{1}_{[S_{j}^{n}(k - 1) = i]}, i \in \{1,2, \dots, D\}$\;
                        $\color{red} S_{j}^{r}(k) \gets \argmin_{i \in \{1,2,\dots, D\}} \frac{-\log h_{j}(i)}{\tilde{V}_{i}^{r}(k)}, j \in \{1,2, \dots, m\}$\;
                    }
                }
                \ElseIf{$k = 2$}{
