...
\section{Zanurzenia/Osadzenia/zagnieżdżenia TODO: potwierdzić nazewnictwo}
    ...
    \subsection{Szkice}
        ...
        \subsubsection*{NodeSketch}
            ...
            \end{equation}
            Wybierając $L$ $(L \ll D)$ niezależnych funkcji haszujących i generując na ich podstawie próbki otrzymujemy szkic $S$ o rozmiarze $L$ wektora $V$. Ponadto, wynikowe szkice zachowują
            ...
            \begin{algorithm}
                ...
                \uIf{$k > 2$}{
                    ...
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        $\tilde{V}^{r}(k) \gets Eq. \ref{eq:node_sketch_3}$\;
                        $S_r(k) \gets Eq. \ref{eq:node_sketch_1}(\tilde{V}^{r}(k))$\;
                    }
                }
                \ElseIf{$k = 2$}{
                    \ForEach{rząd $r$ w $\tilde{A}$}{
                        $S_r(2) \gets Eq. \ref{eq:node_sketch_1}(\tilde{V}^{r})$\;
                    }
                }
                \Return{$S(k)$}
            \end{algorithm}
