%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Add the necessary commands for citations, bibliography data, and bibliography style
...
%JL 
\newcommand{\JL}[1]{\marginpar{\footnotesize{\textcolor{red}{JL:#1}}}}
\newcommand{\PP}[1]{\marginpar{\footnotesize{\textcolor{green}{PP:#1}}}}
...
\ifpdf
 ...
 \usepackage{tikz}
\usetikzlibrary{graphs, matrix, positioning, arrows.meta, shapes.geometric}
\usepackage{caption}
...
\selectfont{\fontsize{26pt}{28pt}\selectfont M}AGISTERSKA\\[2.5cm]%
}}
	\vfill
{\centering
...
\titleShort{Zastosowanie szkiców danych w analizie dużych grafów}  % INFO: krótki tytuł pracy (do zamieszczenia w stopce, sklejony z imieniem i nazwiskiem autora nie powinien zająć więcej niż jedną linijkę)
...
\specjalnosc{Algorytmika (ALG)} % IFO: nazwa kierunku
\promotor{dr inż. Jakub Lemiesz} % INFO: dane promotora 
\kvpl{szkice danych, analiza grafów, strumienie danych} % INFO: słowa kluczowe po polsku
\kven{data sketches, graph analysis, data streams} % INFO: słowa kluczowe po angielsku
...
\SetAlgorithmName{Algorytm}{Algorytm}{Lista algorytmów} % Redefine 'Algorithm' to 'Unicorn'
\begin{document}
