...
\section{Dobór parametru alpha}
    ...
    \JL{Chyba dla algorytmu EdgeSketch ?}
...
\label{sec:performance}
    ...
    %JL , choć odbywa się w czasie $O(1)$, to
    ma znaczący wpływ na czas działania algorytmu. 
    \JL{Trochę zmodyfikowałem powyższe zdanie}
    ...
    \JL{Nie wiem czy nie ciekawszy byłby faktyczny czas rekonstrukcji krawędzi w NodeSketch i EdgeSketch, zwłaszcza dla większych k ?}
...
\]
\JL{Wyjaśnić dokładniej skąd się bierze powyższa formuła}
...
\JL{Warto by chyba jednak zrobić porównanie faktycznych czasów działania algorytmów}
